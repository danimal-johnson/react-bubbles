...
* A token is used for authenticating a user is authorized to access content. It is typically provided by a server after verifying a password. The received token must be provided for each subsequent request to the server.

- [x] What steps can you take in your web apps to keep your data secure?

* One way is to hide secure content behind protected routes and require a server to authenticate a login to provide a JSON Web Token (JWT) that must be sent with every API call.

- [x] Describe how web servers work.

* Servers "listen" for requests on specific port(s)--typically 80 and 443 for web content. When a valid request is received, it replies with
a requested document, file, or information.
